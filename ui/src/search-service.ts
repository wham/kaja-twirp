...
class IndexRequest$Type extends MessageType<IndexRequest> {
    constructor() {
        super("IndexRequest", [
            ...
            { no: 3, name: "additional_copies", kind: "scalar", T: 4 /*ScalarType.UINT64*/, L: 0 /*LongType.BIGINT*/ }
        ]);
    }
    create(value?: PartialMessage<IndexRequest>): IndexRequest {
        const message = { position: 0, additionalCopies: 0n };
        globalThis.Object.defineProperty(message, MESSAGE_TYPE, { enumerable: false, value: this });
        if (value !== undefined)
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                ...
                case /* uint64 additional_copies */ 3:
                    message.additionalCopies = reader.uint64().toBigInt();
                    break;
                default:
                    let u = options.readUnknownField;
                    if (u === "throw")
    ...
    internalBinaryWrite(message: IndexRequest, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        if (message.additionalCopies !== 0n)
